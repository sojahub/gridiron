...
const (
	tomlFilepath = ".gridiron.example.toml"
)
...
var _ = Describe("TestConfig", func() {
	It("should read in the config correctly", func() {
		...
		Expect(config.NodeConfig.AuthVirtualHosts).To(ContainElement("0.0.0.0"))
		Expect(config.NodeConfig.WSHost).To(Equal("0.0.0.0"))
		...
		Expect(config.NodeConfig.GraphQLVirtualHosts).To(ContainElement("0.0.0.0"))
